...
test('Can create new recipe!', t => {
  build().then(() => {
    const data = { recipeTitle: "Spaghetti Bolognese", type: "dinner", ingredients: "spaghetti, mince, tomato", method: "Cook", vegetarian: false };
   createNewEntry(data)
      // .then(getTitles)
      //   .then(title => {
      //     const lastTitle = title[title.length-1]
      //     t.equal(lastTitle.recipetitle, 'Spaghetti Bolognese'),
      //     t.end()
      //   })
        .then(getOneEntry("Spaghetti Bolognese"))
        .then(entry => {
          console.log(entry)
